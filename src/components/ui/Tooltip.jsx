 ...
 */
export function Tooltip({ children, content, side = "top", className, usePortal = false }) {
  ...
  const tooltipContent = (
    <div
      ...
      style={usePortal ? { top: `${position.top}px`, left: `${position.left}px` } : {}}
    >
      <div
        className={cn(
          "px-3 py-2 text-xs font-medium bg-popover text-popover-foreground border border-border rounded-lg shadow-lg whitespace-nowrap max-w-xs",
          className
        )}
      >
        {content}
