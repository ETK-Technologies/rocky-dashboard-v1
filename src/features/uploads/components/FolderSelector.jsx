...
export function FolderSelector({
  ...
  useEffect(() => {
    ...
    const fetchAllFolders = async (parentId = null, visited = new Set()) => {
      try {
        ...
        if (
          ...
          if (additionalPromises.length > 0) {
            ...
            additionalResponses.forEach((additionalResponse) => {
              const additionalFolders = extractFolders(additionalResponse);
        ...
        console.error(`Failed to fetch folders for parentId ${parentId}:`, err);
        return [];
      }
    };
          ...
          <div className="text-center py-8 text-muted-foreground">
            ...
            {allFolders.length > 0 && folderTree.length === 0 && (
              <p className="text-xs mt-2 text-muted-foreground">
                Try selecting Root to move the item to the root folder.
              </p>
            )}
          </div>
        )}
