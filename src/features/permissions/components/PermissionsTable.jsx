...
export function PermissionsTable() {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "150px",
      render: (permission) => (
        <CustomBadge variant="outline">{permission.resource}</CustomBadge>
      ),
    },
    {
      key: "action",
      ...
      width: "120px",
      render: (permission) => (
        <CustomBadge variant="secondary">{permission.action}</CustomBadge>
      ),
    },
    {
      key: "description",
          ...
          >
            ...
            {hasActiveFilters && (
              <span className="ml-1 px-1.5 py-0.5 text-xs bg-primary text-primary-foreground rounded-full">
                {[params.search, params.resource, params.action].filter(Boolean).length}
              </span>
            )}
          </CustomButton>
          <CustomButton
            onClick={() => router.push("/dashboard/roles-permissions/permissions/new")}
            className="flex items-center gap-2"
            size="sm"
          >
      ...
      <DataTable
        ...
        emptyState={{
          ...
          action: hasActiveFilters ? (
            ...
            <CustomButton
              onClick={() =>
                router.push("/dashboard/roles-permissions/permissions/new")
              }
              className="flex items-center gap-2"
            >
              <Plus className="h-4 w-4" />
